...
public class TratamientoService {
    ...
    @Transactional
    public TratamientoDTO create(TratamientoRequestDTO request, HttpServletRequest httpRequest) {
        ...
        TratamientoDTO after = convertToDTO(saved);

        auditService.log(
                AuditAction.CREATE,
                "Tratamiento",
                String.valueOf(saved.getIdTratamiento()),
                null,
                after,
                null,
                httpRequest);
    ...
    @Transactional
    public TratamientoDTO update(Long id, TratamientoRequestDTO request, HttpServletRequest httpRequest) {

        log.info("Actualizando tratamiento con ID: {}", id);

        Tratamiento tratamiento = findTratamientoEntityById(id);

        TratamientoDTO before = convertToDTO(tratamiento);

        if (request.getIdCita() != null &&
                !tratamiento.getCita().getIdCita().equals(request.getIdCita())) {
        ...
        TratamientoDTO after = convertToDTO(updated);

        auditService.log(
                AuditAction.UPDATE,
                "Tratamiento",
                String.valueOf(updated.getIdTratamiento()),
                before,
                after,
                null,
                httpRequest);
    ...
    public void delete(Long id, HttpServletRequest httpRequest) {
        ...
        Tratamiento deleted = tratamientoRepository.save(tratamiento);

        TratamientoDTO after = convertToDTO(deleted);
    ...
    private List<MedicamentoAsignadoDTO> convertMedicamentosToDTO(
